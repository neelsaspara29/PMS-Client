...
const PatientDetail = () => {
    ...
    const getPatientReports = () => {
        ApiGet(`/report/${id}`).then((response: any) => {
            if(response?.data?.success) {
    ...
    const handleAddReport = (value: any) => {
        ...
        ApiPost('/patient/report/add', body).then((response: any) => {
            if(response?.data?.success) {
                handleClose();
                toast.success(response?.data?.message);
                getPatientReports();
    ...
    const handleEditReport = (value: any) => {
      ...
      ApiPost('/patient/report/edit', body).then((response: any) => {
          if(response?.data?.success) {
              handleClose();
              toast.success(response?.data?.message);
              getPatientReports();
  ...
  return (
    <>
      <div className="p-2">
        ...
        <div><Button variant="primary" onClick={() => setShow(true)}>Add Report</Button>
        </div>
        </div>
        ...
        <div>
          ...
          <p>
            <span>Bob : </span> {moementTz(data?.dob).tz("PST")
                                                            .format(
                                                              "MM-DD-YYYY",
                                                            )}
          </p>
        ...
        <Accordion>
            {
                reports?.map((item : any, index: string) => {
                    ...
                    <Accordion.Header>{item?.report_type} : {moementTz(item?.report_date)
                                                            .tz("PST")
                                                            .format(
                                                              "MM-DD-YYYY",
                                                            )}</Accordion.Header>
                    ...
                    <div className="d-flex">
                      ...
                      <Button onClick={() => {
                        setEditId(item?.id);
                        setShow(true);
                        setInitialState({report_type: item?.report_type, description: item?.description, active_status: item?.active_status});
                      }}>Edit </Button>
                      </div>
                      <div>
                        <Badge bg={BADGE_COLOR[item?.active_status]} > {item?.active_status}</Badge>
                      </div>
                    </div>
      ...
      <Modal show={show} onHide={handleClose}>
        ...
        <BForm.Label className='text-color fw-bold' >Report Type</BForm.Label>
        <Field name = "report_type" placeholder="Report Type" component = {MyInput} />
        <p className='error fw-bold'><ErrorMessage name='report_type' /></p>
      </BForm.Group>
       
      <BForm.Group className="mb-3" controlId="formGroupEmail">
        <BForm.Label className='text-color fw-bold'>Description</BForm.Label>
        <Field name = "description" placeholder="Description" component = {MyInput} />
        <p className='error fw-bold'><ErrorMessage name='description' /></p>
      </BForm.Group>   
    </BForm>
    <BForm.Group className="mb-3" controlId="gender">
        <BForm.Label className='me-3 text-color fw-bold'>Status</BForm.Label>
        <BForm.Check
            inline
            label="Draft"
            name="active_status"
            type={"radio"}
            onChange={(e) => {setFieldValue("active_status","draft")}}
          ...
          <BForm.Check
            ...
            name="active_status"
            type={"radio"}
            onChange={(e) => setFieldValue("active_status","publish")}
            checked={values?.active_status == "publish"}
          />
        <p className='error fw-bold'><ErrorMessage name='gender' /></p>
      </BForm.Group>
