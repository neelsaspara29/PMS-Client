...
const Home = () => {
    ...
    const handleAddPatient = (value: TInitialState) => {
      ...
      ApiPost('/patient/add', body).then((response: any) => {
    ...
    const get_all_patient = () => {
        ApiGet('/patient/get/all').then((response: any) => {
            if(response?.data?.success) {
    ...
    return <>
        ...
        <BForm.Label className='text-color fw-bold' >Patient Name</BForm.Label>
        <Field name = "name" placeholder="Name" component = {MyInput} />
        <p className='error fw-bold'><ErrorMessage name='name' /></p>
      </BForm.Group>
      ...
      <BForm.Group className="mb-3" controlId="mobile_number">
        <BForm.Label className='text-color fw-bold'>Mobile Number</BForm.Label>
        <Field name = "mobile_number" placeholder="Mobile Number" component = {MyInput} />
        <p className='error fw-bold'><ErrorMessage name='mobile_number' /></p>
      </BForm.Group>
      <BForm.Group className="mb-3" controlId="gender">
        <BForm.Label className='me-3 text-color fw-bold'>Gender</BForm.Label>
        <BForm.Check
            inline
            label="Male"
            name="gender"
      ...
      <BForm.Group className="mb-3" controlId="dob">
        <BForm.Label className='text-color fw-bold'>Date of Birth</BForm.Label>
        <Field type="date" name = "dob"  component = {MyInput} />
        <p className='error fw-bold'><ErrorMessage name='dob' /></p>
      </BForm.Group>    
    </BForm>
